    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register()
        {
            if (User.IsInRole("Super Administrator"))
            {
                ...
                ViewBag.Employees = db.A_Employees.AsNoTracking().ToList();
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Buses = db.A_Buses.AsNoTracking().ToList();
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Roles = db.AspNetRoles.AsNoTracking().ToList();
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Companies = db.A_Companies.AsNoTracking().ToList();
            }
            else if (User.IsInRole("Administrator"))
            {
                ...
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Employees = db.A_Employees.AsNoTracking().Where(o=> o.Company== logginUser.Company_Id).ToList();
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Buses = db.A_Buses.AsNoTracking().Where(o => o.Company == logginUser.Company_Id).ToList();
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Roles = db.AspNetRoles.AsNoTracking().Where(o=> o.Name != "Super Administrator").ToList();
                db.Configuration.ProxyCreationEnabled = false;
                ViewBag.Companies = db.A_Companies.AsNoTracking().Where(o => o.ID == logginUser.Company_Id).ToList();
            }
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
           ...
           // else if (User.IsInRole("Administrator"))
            ...
            if (ModelState.IsValid)
            {
                ApplicationUser user = new ApplicationUser { UserName = model.UserName, Email = model.Email,PhoneNumber = model.PhoneNumber,Company_Id=model.Company_Id,Bus_Id=model.Bus_Id };
                IdentityResult result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    var _result = await this.UserManager.AddToRoleAsync(user.Id, model.UserRole);
                    ...
                    {
                        var exists = db.A_Employees.FirstOrDefault(o => o.Employee_ID == model.NameOfAccountHolder);
                        if(exists != null)
                        {
